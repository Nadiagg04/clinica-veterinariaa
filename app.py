...
from src.servicios.servicio_producto import ServicioProducto
...
# ===================== PRODUCTOS =====================
    ...
    try:
        productos = servicio_producto.obtener_todos()
        if productos:
            df_productos = pd.DataFrame(
                [(p.id, p.nombre, p.precio, p.stock) for p in productos],
    ...
    with st.expander("Agregar Producto"):
        ...
        if st.button("Guardar Producto"):
            if nombre and precio is not None and stock is not None:
                try:
                    producto = servicio_producto.crear_producto(nombre, precio, stock)
